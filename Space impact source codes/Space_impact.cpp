...
void iDraw()
{
...
else if (gamestate==2)
    {
        ...
        iSetColor(255,255,255);
        iText(10,1000,sc,GLUT_BITMAP_TIMES_ROMAN_24);

        for(int i=0;i<(health/10);i++)
        {
            iShowBMP2(1300+60*i,1000,"life.bmp",0);

        }

        iShowBMP2(hpack_x,hpack_y,"hpack.bmp",0);
    ...
    else if (gamestate==4)
    {
        ...
        iSetColor(0,0,0);
        iText(1000,500,sc,GLUT_BITMAP_TIMES_ROMAN_24);
    ...
    else if (gamestate==5)
    {
        ...
        iShowBMP(0,0,"lv.bmp");
        iSetColor(255,255,255);
        iText(50,700,sc,GLUT_BITMAP_TIMES_ROMAN_24);
    ...
    {
        iClear();
        iShowBMP(pic_x,pic_y,"about.bmp");
    }
    else if (gamestate==8)
    {
        iClear();
        iShowBMP(pic_x,pic_y,"settings.bmp");
    }
...
void iKeyboard(unsigned char key)
{
    ...
    if (key == ' ')
    {
        if(gamestate==4 || gamestate==6 || gamestate==5 || gamestate==7 || gamestate==8)
        {
            gamestate=1;
        ...
        for(int j=0;j<amx;j++)
            {
                ...
                if (health<=90 && gamestate==2 && ss1_x+120>=hpack_x && ss1_x<=hpack_x+50 && ss1_y+142>=hpack_y && ss1_y<=hpack_y+50)
                {
                    health+=10;
                    hpack_y=2000;
                }
                if (boss1==1 && ss1_x+120>=bb_x && ss1_x<=bb_x+614 && ss1_y+142>=bb_y && ss1_y<=bb_y+36)
                {
                    health-=30;
                    bb_y=2000;
                }
...
int main()
{
    ...
    iSetTimer(2,fire);
    iSetTimer(20,boss);
    iSetTimer(10000,gift);
    iSetTimer(5000, bossbeam);
